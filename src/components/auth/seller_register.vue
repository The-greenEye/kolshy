  ...
  <div class="register-page">
    ...
    <div class="otp-container bg-light" :class="{ 'otp-active': showOtp }">
      ...
      <div class="otp-inputs">
        <input v-for="i in 6" :key="i" v-model="otp[i - 1]" @input="handleOtpInput($event, i - 1)" @keydown.delete="handleOtpDelete($event, i - 1)" ref="otpInputs" class="otp-input" type="text" maxlength="1" />
      </div>
      ...
      <div class="rounded p-2 bg-light form-sign">
        <div class="d-flex flex-column justify-content-center align-items-start">
          ...
          <form method="post" class="w-100 p-0" @submit.prevent="sendForm()">
            ...
            <small v-if="v$.Password.$error" style="font-size: 12px; font-weight: bold; color: red">Password must be at least 8 characters with uppercase, lowercase, number, and special character</small>
            <div style="border: none; border-bottom: 1px solid #e51742" class="input-group w-100 justify-content-between align-items-end mb-0">
            ...
            <div>
              <small v-if="v$.shopName.$error" style="font-size: 12px; font-weight: bold; color: red">Please enter a shop name</small>
              <div style="border: none; border-bottom: 1px solid #e51742" class="input-group w-100 justify-content-between align-items-end mb-4">
                <input type="text" class="bg-transparent w-75 p-2" v-model="shopName" style="border: none; outline: none; font-weight: 500" placeholder="Shop Name" aria-label="Shop" @blur="v$.shopName.$touch()" required />
              </div>

              <div style="border: none; border-bottom: 1px solid #e51742" class="input-group w-100 justify-content-between align-items-end mb-4 mt-4">
                <input type="text" class="bg-transparent w-75 p-2" v-model="street" style="border: none; outline: none; font-weight: 500" placeholder="Street" aria-label="Street" required />
              </div>

              <div style="border: none; border-bottom: 1px solid #e51742" class="input-group w-100 justify-content-between align-items-end mb-4">
                <input type="text" class="bg-transparent w-50 p-2" v-model="city" style="border-right: 1px solid gray; border-top: none; border-left: none; border-bottom: none; outline: none; font-weight: 500" placeholder="City" aria-label="City" required />
                <input type="text" class="bg-transparent w-50 p-2" v-model="ZIP" style="border: none; outline: none; font-weight: 500" placeholder="ZIP Code" aria-label="ZIP" />
              </div>
              ...
              <div style="border: none; border-bottom: 1px solid #e51742" v-for="(lang, index) in languages" :key="index" class="input-group w-100 justify-content-between align-items-end mb-4">
                <input type="text" class="bg-transparent w-75 p-2" v-model="about[lang]" style="border: none; outline: none; font-weight: 500" :placeholder="`About in ${lang}`" aria-label="About" />
              </div>

              <small v-if="v$.phone.$error" style="font-size: 12px; font-weight: bold; color: red">Please enter a valid phone number (at least 8 digits)</small>
              <div style="border: none; border-bottom: 1px solid #e51742" class="input-group w-100 justify-content-between align-items-end mb-4">
                <input type="tel" class="bg-transparent w-75 p-2" v-model="phone" style="border: none; outline: none; font-weight: 500" placeholder="Phone Number" aria-label="Phone" @blur="v$.phone.$touch()" required />
              </div>

              <div>
                <div class="d-flex align-items-start m-2">
                  <input type="checkbox" name="type-account" value="accept-ruls" v-model="accept" style="outline: none; margin: 0 8px" required />
...
export default {
  ...
  validations() {
    return {
      ...
      Password: {
        ...
        // Add custom validation for password complexity if needed
      },
      Password_confirmation: {
        required,
  ...
  methods: {
      ...
      try {
        ...
        if (error.response) {
          if (error.response.status === 422) {
            errorMessage = "Validation errors: " + Object.values(error.response.data.errors).flat().join(", ");
          } else {
    ...
    async getFildes() {
      try {
        const response = await axios.get("https://back.kolshy.ae/api/seller/register/form-data");
        if ([200, 201].includes(response.status)) {
          this.languages = response.data.data.languages;
        } else {
          this.toast.error("Failed to load language options");
        }
      } catch (error) {
    ...
    async submitOtp() {
      ...
      try {
        const verificationResponse = await axios.post(
          ...
          {
            headers: {
              Authorization: `Bearer ${localStorage.tokenkolshyvendor}`,
            },
          }
        );
        ...
        if ([200, 201].includes(verificationResponse.status)) {
          ...
          localStorage.setItem("store_name", this.shopName);
          // localStorage.setItem("url", this.shopUrl); // shopUrl wasn't defined in your original code
